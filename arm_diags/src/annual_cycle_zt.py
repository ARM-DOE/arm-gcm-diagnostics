...
#    test_var_season=np.empty([len(variables),len(seasons)])*np.nan
    test_file = glob.glob(os.path.join(test_path,'*'+test_model+'*diurnal*.nc')) #read in monthly test data
    ...
    for j, variable in enumerate(variables): 
        try:
            ...
            with file(output_path+'/metrics/'+variable+'_test_diurnal_climo.csv', 'w') as outfile:
...
#    obs_var_season=np.empty([len(variables),len(seasons)])*np.nan
    ...
    for j, variable in enumerate(variables): 
        try:
            ...
            with file(output_path+'/metrics/'+variable+'_obs_diurnal_climo.csv', 'w') as outfile:
                outfile.write('# Array shape: {0}'.format(var_2d.shape)+' as (month, hours, vertical levels)\n')
                mon_id=0
                for data_slice in var_2d:
                    #outfile.write('# New slice\n')
...
def annual_cycle_zt_plot(parameter):
    ...
    month=seasons#['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec']
    for j, variable in enumerate(variables):
        test_data = np.loadtxt(output_path+'/metrics/'+variable+'_test_diurnal_climo.csv')
        test_data = test_data.reshape((12,8,37))
#        mmm_data = genfromtxt(output_path+'/metrics/'+variable+'_mmm_annual_cycle.csv')
        obs_data = np.loadtxt(output_path+'/metrics/'+variable+'_obs_diurnal_climo.csv')
...
#        cmip_data = genfromtxt(output_path+'/metrics/'+variable+'_cmip_annual_cycle.csv')
        ...
        for index in range(len(seasons)):
            ...
            fig3.savefig(output_path+'/figures/'+seasons[index]+'_'+variable+'_diff.png')
            plt.close('all')
        ...
        fig0.savefig(output_path+'/figures/ANN_'+variable+'_diff.png')
        plt.close('all')
