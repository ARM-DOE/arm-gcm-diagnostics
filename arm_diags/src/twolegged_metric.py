...
def twolegged_metric_plot(parameter):
    ...
    try:
        # Open dataset with xarray and decode_times=False to prevent time parsing issues
        ds_obs = xr.open_dataset(obs_file[0], decode_times=False)
        
        # Extract variables
        sh0 = ds_obs['SH'].values
        lh0 = ds_obs['LH'].values
        LCL0 = ds_obs['LCL'].values
        sm_ebbr0 = ds_obs['soil_moisture_ebbr'].values
        sm_swats0 = ds_obs['soil_moisture_swats'].values
    ...
    try:
        # Open dataset with xarray and decode_times=False to prevent time parsing issues
        ds_test = xr.open_dataset(test_file[0], decode_times=False)
        
        # Extract variables
        sh1 = ds_test['SH'].values
        lh1 = ds_test['LH'].values
        LCL1 = ds_test['LCL'].values
        
        print('sh1: ', np.nanmin(sh1), np.nanmax(sh1))
        print('lh1: ', np.nanmin(lh1), np.nanmax(lh1))
        print('LCL1: ', np.nanmin(LCL1), np.nanmax(LCL1))
    ...
    for im in range(nt_10day):
        for iseason in range(nseasons):
            tmp1 = sh[:, iseason]
            tmp2 = np.nanmean(tmp1[10*im:10*im+10])
